...
import { Collapse, ButtonGroup, Button, FormGroup, Label } from 'reactstrap';
...
export default class TaskListItem extends React.Component<Props, State> {  
    ...
    hideContent = () => {
        setTimeout(() => {
            if(!this.state.open && this.state.contentVisible)
                this.setState({contentVisible:false, time:moment({hour: 8})})
        }, 500);
    }
    ...
    setTaskState = (state:TaskState) => (event:any) => {
        const currentState = this.props.task.state
        const newState = currentState == state ? null : state
        this.props.onActionPress(TaskActions.setState,{state:newState})
    }
    onTaskClick = (event) => {
        const open = this.state.open
        const update:Partial<State> = {open:!open}
        if(!open)
            update.contentVisible = true
        const func = open ? this.hideContent : undefined        
        this.setState(update as State, func)
    }
    ...
    onAddCommentActionPress = (action: StatusActions, extra?: Object, completion?: (success: boolean) => void) => {
        console.log("onAddCommentActionPress", action, extra)
                    ...
                    <Collapse isOpen={this.state.open}>
                        {this.state.contentVisible &&
                            <div className="collapse-body p-2">
                                ...
                                <FormGroup>
                                    <Label>{translate("project.module.menu.comment.title")}</Label>
                                    <StatusComposerComponent 
                                        canUpload={true}
                                        canMention={true}
                                        canComment={true}
                                        onActionPress={this.onAddCommentActionPress}
