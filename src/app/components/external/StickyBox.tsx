...
let passiveArg:{passive: boolean} | boolean = false;
try {
...
type DefaultProps = {
    onChangeMode: (oldMode:string, newMode:string) => void,
    offsetTop: number,
    offsetBottom: number,
    bottom: boolean,
}
...
export default class StickyBox extends React.Component<Props,{}> {
    node:any = null
    scrollPane:any = null
    latestScrollY:number = 0
    rosp:ResizeObserver
    ropn:ResizeObserver
    ron:ResizeObserver
    scrollPaneOffset:number = 0
    mode:string
    offset = 0
    viewPortHeight = 0
    nodeHeight = 0
  ...
  changeMode = (newMode) => {
    ...
    if (newMode === "relative") {
      ...
      this.node.style.top = `${Math.round(this.offset)}px`;
    } else {
      this.node.style.position = stickyProp;
      if (newMode === "stickyBottom") {
        this.node.style.top = `${Math.round(this.viewPortHeight - this.nodeHeight - offsetBottom) }px`;
      } else {
        this.node.style.top = `${Math.round(offsetTop)}px`;
      }
    }
