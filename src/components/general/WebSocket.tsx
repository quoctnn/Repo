...
class WebSocket extends React.Component<Props, {}> {
    ...
    componentDidMount()
    {
        let domain = this.props.availableApiEndpoints[this.props.apiEndpoint].endpoint
        let url = domain + Routes.WEBSOCKET
        this.stream = new ReconnectingEventSource(url);
        this.stream.addEventListener('message', function (e) {
            console.log(e.data);
        }, false);

        this.stream.addEventListener('stream-reset', function (e) {
...
export default connect(mapStateToProps, null)(WebSocket);